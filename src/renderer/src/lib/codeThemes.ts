...
import type { CodeThemeId } from '@shared/settings'
...
const PALETTE_TO_VAR: Record<keyof AppPalette, (typeof APP_VAR_KEYS)[number]> = {
...
export function applyAppPalette(palette: AppPalette | null): void {
  if (typeof document === 'undefined') return
  const root = document.documentElement
  if (!palette) {
    APP_VAR_KEYS.forEach((k) => root.style.removeProperty(k))
    return
  }
  ;(Object.keys(palette) as (keyof AppPalette)[]).forEach((k) => {
    root.style.setProperty(PALETTE_TO_VAR[k], palette[k])
  })
}
...
const draculaDarkPrism: PrismTheme = {
  ...
  styles: [
    ...
    { types: ['selector', 'attr-name', 'string', 'char', 'builtin', 'inserted'], style: { color: '#f1fa8c' } },
...
const draculaLightPrism: PrismTheme = {
  ...
  styles: [
    { types: ['comment', 'prolog', 'doctype', 'cdata'], style: { color: '#635196', fontStyle: 'italic' } },
    ...
    { types: ['selector', 'attr-name', 'string', 'char', 'builtin', 'inserted'], style: { color: '#3a7d34' } },
...
function colorsFromPrism(prism: PrismTheme, overrides: Partial<CodeThemeColors> = {}): CodeThemeColors {
...
function lightColorsFromPrism(prism: PrismTheme, overrides: Partial<CodeThemeColors> = {}): CodeThemeColors {
