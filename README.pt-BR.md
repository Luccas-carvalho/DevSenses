...
[Download (mac)](#-instalar) · [Como funciona](#-como-funciona) · [Features](FEATURES.md) · [Contribuir](CONTRIBUTING.md) · [Roadmap](MVP_PLAN.md)
...
Baixe o `.dmg` da [última release](https://github.com/Luccas-carvalho/DevSenses/releases) e arrasta pra Applications.

> Os builds ainda **não são assinados** (code signing em andamento). Na primeira
> execução o macOS mostra "app não verificado". Pra abrir:
> **clique com o botão direito no app → Abrir → Abrir** (só na 1ª vez).
> Se o macOS bloquear mesmo assim: Ajustes do Sistema → Privacidade e Segurança →
> "Abrir mesmo assim".

### Build da source (qualquer plataforma)
...
> Releases oficiais hoje cobrem macOS. Builds de Windows/Linux saem da source
> (ainda em validação). Contribuições pra testá-los são bem-vindas.

## 🔒 Privacidade

- **Teu código nunca vai pra um servidor nosso** — não existe backend do DevSenses.
- O **diff** só é enviado pro provider de IA que **tu** escolheu (com a tua chave). No Ollama, nem sai da máquina.
- **Telemetria vem desligada por padrão** (opt-in). Quando ligada, os eventos ficam num SQLite **local** e podem ser apagados em Configurações → Privacidade.
...
PRs e issues são bem-vindos — especialmente bug reports com um diff que reproduz o
problema. Comece pelo [`CONTRIBUTING.md`](CONTRIBUTING.md) (setup, padrões, fluxo de
PR). Participantes seguem o [`CODE_OF_CONDUCT.md`](CODE_OF_CONDUCT.md). Vulnerabilidade
de segurança: ver [`SECURITY.md`](SECURITY.md) (não abra issue pública).
